...
End If
If judgedbottom>Timer Then 
activeda=activeda+3.5
EndIf
If judgedtop>Timer Then 
activedb=activedb+3.5
EndIf
...
'topzzz3=topzzz3/2
flatx=ship(pointidx).x*((70*.55)*((topzzz3*5)))/(ship(pointidx).z+zoomlevel)
...
'shade1=.5-SHADEA*4
flaty=ship(pointidx).y*((70*.55)*(topzzz3*5))/(ship(pointidx).z+zoomlevel)
...
'     END IF34
flatz=ship(pointidx).Z*((128*8)*1)*(topzzz3*5)
If judgedtop<Timer then
newx = flatx'newx'
...
flatx=ship(pointidx).x*((70*.55)*((bottomzzz3*5)))/(ship(pointidx).z+zoomlevel)
'IF SHIP2ROTATEX>=0 THEN 
    SHADE2=1.5+ship(POINTIDX).X*COS(SHIP2ROTATEX+(PIEYEDI/2))
'ELSEIF SHIP2ROTATEX<=0 THEN    
'    SHADE2=1.5+ship(POINTIDX).X*COS(SHIP2ROTATEX+(PIEYEDI/2))
'     END IF
flaty=ship(pointidx).y*((70*.55)*(bottomzzz3*5))/(ship(pointidx).z+zoomlevel)
...
'     END IF
flatz=ship(pointidx).Z*((128*8)*1)*(bottomzzz3*5)
If judgedbottom<Timer Then
 newx = flatx'newx'
...
if pointidx3=1 and judgedtop>timer then
    rainbow=rgb(d*casual,n*casual,e*casual)'rainbow=rgb(255*casual,255*casual,0)
    end if
if pointidx3=2 and judgedbottom>timer then
        rainbow=rgb(d*casual2,n*casual2,e*casual2)'rainbow=rgb(255*casual2,255*casual2,0)
end if
if pointidx3=1 Then
...
'asm
 '   popf
  ...
  dim as integer timber3
     ...
     Const SOUND_FILE = "burghrecords.mp3"'crowdnoise.aif"'nature.mp3"
...
Dim temptexty As string
Dim texty As String="NEMESISDONOTBE"
'For countery=1 To 13
'	texty=texty+texty
'Next
texty=UCase(texty)
'ABCdefgZAHOLYGODAZ
...
dim as integer astrocount=0
 ...
 '   picture2 = ImageCreate( 1279, 719, RGB(0, 0,0) ) 
   ...
   for y3=1 to 720/2
        line(1,y3)-(1279,y3),rgb(0,255*(y3*100/(720/2)/100),192*(y3*100/(720/2)/100))',255*(y3*100/(720/2)/100))
    next
...
'paint(hammerx,hammery+89),rgb(255,255,255),rgb(255,255,254)
hammerx2=((1280)/2)+(cos(flat2*2+PIEYEDI)*(1280*.25*.75))
hammery2=((720)/2)+(sin(flat2*2+PIEYEDI)*(720*.25*.75))
...
radcountlinein=radcountlinein-1
 For a = 0 To 360 step +.5'33'1'.5'3333333'25
        x = Cos(a) * radcount
        y = Sin(a) * radcount
        x1=cos(a) * radcountlinein
        y1=sin(a) * radcountlinein
...
'paint(((1280)/2)+cos(flat1)/pee,((720)/2)+sin(flat1)/pee),rgb(255, 255,255),RGB(255,255,255)
hammerx2=((1280)/2)+(cos(flat2*2)*(1280*.25*.75))
hammery2=((720)/2)+(sin(flat2*2)*(720*.25*.75))
'screenlock
...
for radcount=radcountlineout to (xfive*2)
radcountlinein=radcountlinein-1
 For a = 0 To 360 step +.5'.5'3333333'25
        x = Cos(a) * radcount
        y = Sin(a) * radcount
        x1=cos(a) * radcountlinein
        y1=sin(a) * radcountlinein
...
temptexty=""
For COUNTON=1 To 27
For countery=Len(texty) To 1 Step -2
	temptexty=temptexty+Mid(texty,countery,1)
	'temptexty=Mid(TEMPTEXTY,Len(TEMPTEXTY),1)+Mid(TEMPTEXTY,1,Len(TEMPTEXTY)-7)
Next
texty=temptexty
temptexty=""
Next
...
if game2=0 then
    flat1=flat1+(pieyedi*2/(360*(pieyedi)))

if flat1>pieyedi*2 then flat1=0+(pieyedi*2/(360*(pieyedi)))
end if
...
pressaflg=0
  ...
  ' stream3=FSOUND_Stream_Open(SOUND_FILE3, FSOUND_MPEGACCURATE, 0, 0)
    ...
    '    FSOUND_Stream_Play(FSOUND_FREE, stream3)       'sleep
       ...
       '  fsound_setvolume(stream,vol255)
zcount=0
pressed=0
pressaflg=0
readyflg=0
...
dim as SINGLE shade1,shade2
 If judgedtop>timer and judgedtop2<Timer-.66 then judgedtop2=Timer+.33'int(timer)+.5
If judgedbottom>timer and judgedbottom2<Timer-.66 then judgedbottom2=Timer+.33'int(timer)+.5
...
if pointidx3=1 then
 ...
 if judgedtop>timer then rainbow=rgb(255,127,0)
 screenlock
...
'screenlock
 ...
 if judgedbottom>timer then rainbow=rgb(255,127,0)
 screenlock
...
'if topzzz3<.5 then topzzz3=topzzz3*2  
'topzzz3=topzzz3/2
flatx=shiprightside(pointidx).x*((70*.55)*((topzzz3*5)))/(shiprightside(pointidx).z+zoomlevel)
'SHADEA=ship1rotatex*100/(pieyedi/2)/100
'IF SHIP1ROTATEX>=0 THEN 
...
':shade1=shade1-(shade1*2)
'shade1=.5-SHADEA*4
flaty=shiprightside(pointidx).y*((70*.55)*((topzzz3*5)))/(shiprightside(pointidx).z+zoomlevel)
'IF SHIP1ROTATEY>0 THEN  
'    SHADEA=SHIP(POINTIDX).X*COS(SHIP1ROTATEY+(PIEYEDI/2))
'ELSEIF SHIP1ROTATEY<0 THEN
'     SHADEA=SHIP(POINTIDX).X*COS(SHIP1ROTATEY+(PIEYEDI/2))
...
flatz=shiprightside(pointidx).Z*((128*8)*1)*(topzzz3*5)
If judgedtop<Timer Then
 newx = flatx'newx'
newy = (sin(ship1rotatey) * flatz) + (cos(ship1rotatey) * flaty)
newz = (cos(ship1rotatey) * flatz) - (sin(ship1rotatey) * flaty)
...
'if bottomzzz3<.25/2 then bottomzzz3=.25/2
'if bottomzzz3>.25 then bottomzzz3=.25'-.1
flatx=shiprightside(pointidx).x*((70*.55)*((bottomzzz3*5)))/(shiprightside(pointidx).z+zoomlevel)
'IF SHIP2ROTATEX>=0 THEN 
    SHADE2=3+SHIPrightside(POINTIDX).X*COS(SHIP2ROTATEX+(PIEYEDI/2))
'ELSEIF SHIP2ROTATEX<=0 THEN  
'    SHADE2=3+SHIPrightside(POINTIDX).X*COS(SHIP2ROTATEX+(PIEYEDI/2))
'     END IF
flaty=shiprightside(pointidx).y*((70*.55)*((bottomzzz3*5)))/(shiprightside(pointidx).z+zoomlevel)
'IF SHIP2ROTATEY>0 THEN 
'    SHADEB=SHIP(POINTIDX).X*COS(SHIP2ROTATEY+(PIEYEDI/2))
'ELSEIF SHIP2ROTATEY<0 THEN
'     SHADEB=SHIP(POINTIDX).X*COS(SHIP2ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shiprightside(pointidx).Z*((128*8)*1)*(bottomzzz3*5)'/((720*2+(720/2)))'/shiprightside(pointidx).Z)'*1000'*1'*(.25-(.25/3)+(topzzz3*3))'/(ship(pointidx).Z+zoomlevel)'((3)
If judgedbottom<Timer Then
 newx = flatx'newx'
newy = (sin(ship2rotatey+pieyedi) * flatz) + (cos(ship2rotatey+pieyedi) * flaty)
newz = (cos(ship2rotatey+pieyedi) * flatz) - (sin(ship2rotatey+pieyedi) * flaty)
...
'if topzzz3<.5 then topzzz3=topzzz3*2  
'topzzz3=topzzz3/2
flatx=shipleftside(pointidx).x*((70*.55)*((topzzz3*5)))/(shipleftside(pointidx).Z+zoomlevel)
'SHADEA=ship1rotatex*100/(pieyedi/2)/100
'IF SHIP1ROTATEX>=0 THEN 
...
':shade1=shade1-(shade1*2)
'shade1=.5-SHADEA*4
flaty=shipleftside(pointidx).y*((70*.55)*((topzzz3*5)))/(shipleftside(pointidx).Z+zoomlevel)
'IF SHIP1ROTATEY>0 THEN 
'    SHADEA=SHIP(POINTIDX).X*COS(SHIP1ROTATEY+(PIEYEDI/2))
'ELSEIF SHIP1ROTATEY<0 THEN
'     SHADEA=SHIP(POINTIDX).X*COS(SHIP1ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shipleftside(pointidx).Z*((128*8)*1)*(topzzz3*5)'/((720*2+(720/2)))'/shipleftside(pointidx).Z)/((720*2+(720/2))'/shipleftside(pointidx).Z)'*1000'*1'*(.25-(.25/3)+(topzzz3*3))'/(ship(pointidx).Z+zoomlevel)'((3)
If judgedtop<Timer Then
 newx = flatx'newx'
newy = (sin(ship1rotatey) * flatz) + (cos(ship1rotatey) * flaty)
newz = (cos(ship1rotatey) * flatz) - (sin(ship1rotatey) * flaty)
...
'if bottomzzz3<.25/2 then bottomzzz3=.25/2
'if bottomzzz3>.25 then bottomzzz3=.25'-.1
flatx=shipleftside(pointidx).x*((70*.55)*((bottomzzz3*5)))/(shipleftside(pointidx).Z+zoomlevel)
'IF SHIP2ROTATEX>=0 THEN 
    SHADE2=3+SHIPleftside(POINTIDX).X*COS(SHIP2ROTATEX+(PIEYEDI/2))
'ELSEIF SHIP2ROTATEX<=0 THEN  
'    SHADE2=3+SHIPleftside(POINTIDX).X*COS(SHIP2ROTATEX+(PIEYEDI/2))
'     END IF
flaty=shipleftside(pointidx).y*((70*.55)*((bottomzzz3*5)))/(shipleftside(pointidx).Z+zoomlevel)
'IF SHIP2ROTATEY>0 THEN 
'    SHADEB=SHIP(POINTIDX).X*COS(SHIP2ROTATEY+(PIEYEDI/2))
'ELSEIF SHIP2ROTATEY<0 THEN
'     SHADEB=SHIP(POINTIDX).X*COS(SHIP2ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shipleftside(pointidx).Z*((128*8)*1)*(bottomzzz3*5)'/((720*2+(720/2)))'/shipleftside(pointidx).Z)/((720*2+(720/2))'/shipleftside(pointidx).Z)'*1000'*1'*(.25-(.25/3)+(topzzz3*3))'/(ship(pointidx).Z+zoomlevel)'((3)
If judgedbottom<Timer Then
 newx = flatx'newx'
newy = (sin(ship2rotatey+pieyedi) * flatz) + (cos(ship2rotatey+pieyedi) * flaty)
newz = (cos(ship2rotatey+pieyedi) * flatz) - (sin(ship2rotatey+pieyedi) * flaty)
...
'     SHADEB=SHIP(POINTIDX).X*COS(SHIP2ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shiprightside(pointidx).Z*((128*8)*1)*(bottomzzz3*5)
...
'     SHADEA=SHIP(POINTIDX).X*COS(SHIP1ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shipleftside(pointidx).Z*((128*8)*1)*(topzzz3*5)'/((720*2+(720/2)))'/shipleftside(pointidx).Z)'*1000'*1'*(.25-(.25/3)+(topzzz3*3))''((3)
...
'     SHADEB=SHIP(POINTIDX).X*COS(SHIP2ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shipleftside(pointidx).Z*((128*8)*1)*(bottomzzz3*5)'/((720*2+(720/2)))'/shipleftside(pointidx).Z)'*1000'*1'*(.25-(.25/3)+(topzzz3*3))''((3)
 newx = flatx'newx'
newy = (sin(ship2rotatey+pieyedi) * flatz) + (cos(ship2rotatey+pieyedi) * flaty)
newz = (cos(ship2rotatey+pieyedi) * flatz) - (sin(ship2rotatey+pieyedi) * flaty)
 flatx=newx
...
'     SHADEA=SHIP(POINTIDX).X*COS(SHIP1ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shiprightside(pointidx).Z*((128*8)*1)*(topzzz3*5)'/((720*2+(720/2)))'/shiprightside(pointidx).Z)'*1000'*1'*(.25-(.25/3)+(topzzz3*3))''((3)
...
'     SHADEB=SHIP(POINTIDX).X*COS(SHIP2ROTATEY+(PIEYEDI/2))
'     END IF
flatz=shiprightside(pointidx).Z*((128*8)*1)*(bottomzzz3*5)'/((720*2+(720/2)))'/shiprightside(pointidx).Z)'*1000'*1'*(.25-(.25/3)+(topzzz3*3))''((3)
...
' if tiesaucer=1 then
   If buttons2=16 Or buttons2=32 Then
...
'        bottomgun3(bottomxxx,bottomyyy)=int(bottomzzz*5*12)
       ...
       ' if bottomgn3=0 then bottomgn3=int(bottomzzz*5*12)
'    else
If buttons=16 Or buttons=32 Then
...
'if topzzz2<0 then topzzz2=topzzz2-(topzzz2*2)
 ...
 for hondal=1 to int(rnd*25)+1*(25/2)
                      hondax=rnd*64
                       honday=rnd*64
                       screenlock
                       circle(bottomxxx+32-hondax,bottomyyy+32-honday),rnd*256/2*(bottomzzz*5),rgb(255,(255*rnd),0),,,,F
                               screenunlock
'judgedbottom=timer+1
...
for hondal=1 to int(rnd*25)+1*(25/2)
                      hondax=rnd*64
                       honday=rnd*64
                       screenlock
                       circle(topxxx+32-hondax,topyyy-32+32-honday),rnd*256/2*(topzzz*5),rgb(255,(255*rnd),0),,,,F
                               screenunlock
'judgedtop=timer+1
...
'bottomfired=0
    ...
    if bottomgun(tempgunx2,tempguny2,tempgunz)>0 and bottomgun(tempgunx2,tempguny2,tempgunz)<1 then
...
topguny(calca)=0
    ...
    if topgun(tempgunx,tempguny,tempgunz)>0 and topgun(tempgunx,tempguny,tempgunz)<1 then
...
IF astrox(ASTROCNT)>TEMPGUNX-((64)) AND astrox(ASTROCNT)<TEMPGUNX+((64)) THEN
 ...
 for hondal=1 to int(rnd*25)+1*(25*6/3)
                      hondax=rnd*64
                       honday=rnd*64
                       screenlock
...
'dim as SINGLE calc1
'counter_end()
if game2=0 then timber3=30
if   (game2=1 or game2=12)  then timber3=30
if game2=2 or game2=4 or game2=6 or game2=11 then timber3=30
